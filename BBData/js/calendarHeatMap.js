...
/*function oneCalendarPerYear:
  json: the json data
  width: width of the SVG
  height: height of the SVG
  id: id of the SVG
  parentDiv: id ot the div containing the SVG in order to append it in the SVG
  Description: This function creates a div element and an id for each year
              and then draw one calendar heat map for each sensor.
*/
function oneCalendarPerYear(json, width, height, id, parentDiv){


// If there are already calendar, remove it and replace it by the new ones
  elem= document.getElementsByClassName("deleteCalendar");
  while(elem[0]){
    elem[0].parentNode.removeChild(elem[0]);
  }

// Push all values in an array in order to compute average per day for all sensors
  var allValues=[];
...
// Put all mean of each day in the array to compute max, min, median, mean of mean
  var allValuesMean=[];
    for(year in nestBeforeCompletion){
        for(var month in nestBeforeCompletion[year].values){
          for(var measure in nestBeforeCompletion[year].values[month].values){
            allValuesMean.push(nestBeforeCompletion[year].values[month].values[measure].value.value);
          }
        }
      }

// Max,min, mean, median for scales
  ...
  for(sensors in nestBeforeCompletion){

  // Creation of the div
    div = document.createElement("div");
    div.setAttribute("id","divCalendar"+sensors);
    div.setAttribute("class","deleteAll deleteCalendar");
  ...
  // Creation of a unique id for each graph
    ...
    nestBeforeCompletion[sensors]["unit"]={"symbol":json[0].unit.symbol};

    var array=[];
    array.push(nestBeforeCompletion[sensors]);

  // Create one boxPlot for each sensor
    calendarHeatMap(array, width, height, uniqueId, div,maxValue,minValue,mean,median);
  }
}
...
/*function calendarHeatMap:
  ...
  height: height of the SVG
  id: id of the SVG
  parentDiv: id of the div containing the SVG in order to append it inside
  maxValue: maximum value for the scale
  minValue: minimum value for the scale
  mean: mean value for the scale
  median: median value for the scale
  Description: Draw a calendar heat map for the given year given by oneCalendarPerYear() function
*/
function calendarHeatMap(json, width, height, id, parentDiv,maxValue,minValue,mean,median){
...
// store json in another name
  nestBeforeCompletion=json;
...
// Append the days of the calendar
